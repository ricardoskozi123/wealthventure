...
class Activity(db.Model):
    ...
    # Relationships - use simple string references to avoid circular imports
    ...
    def log(action_type, description, user=None, lead=None, target_type=None, target_id=None, data=None):
        ...
        """
        # Safety checks to avoid NULL primary key issues
        user_id = None
        if user and hasattr(user, 'id') and user.id is not None:
            user_id = user.id
        
        lead_id = None
        if lead and hasattr(lead, 'id') and lead.id is not None:
            lead_id = lead.id
