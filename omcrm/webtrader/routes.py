...
@webtrader.route("/chart_data/<symbol>")
@login_required  
def get_chart_data(symbol):
    """
    🎯 CUSTOM TRADINGVIEW DATA FEED
    Serves historical price data from our database to ensure chart matches trading prices
    """
    try:
        # Get query parameters for TradingView
        resolution = request.args.get('resolution', '1D')  # 1D, 1H, 15, etc.
        from_timestamp = request.args.get('from', None)
        to_timestamp = request.args.get('to', None)
        ...
        # For now, return current price as a simple OHLC bar
        # In production, you'd query historical price data from your database
        current_price = float(instrument.current_price) if instrument.current_price else 0.0
        
        # Generate simple OHLC data (same price for all values for now)
        # This ensures the chart shows exactly what users will trade at
        chart_data = {
            'time': [int(datetime.utcnow().timestamp())],
            'open': [current_price],
            'high': [current_price * 1.001],  # Slight variation for chart
            'low': [current_price * 0.999],
            'close': [current_price],
            'volume': [1000]
        }
        
        logging.info(f"📊 Custom chart data for {symbol}: ${current_price}")
        ...
        logging.error(f"❌ Chart data error for {symbol}: {str(e)}")
        return jsonify({'error': str(e)}), 500

@webtrader.route("/realtime_price/<symbol>")
@login_required
def get_realtime_price_for_chart(symbol):
    """
    🔴 REAL-TIME PRICE FOR CHART
    Returns current price from database for live chart updates
    """
    try:
        instrument = TradingInstrument.query.filter_by(symbol=symbol).first()
        if not instrument:
        ...
        current_price = float(instrument.current_price) if instrument.current_price else 0.0
        change = float(instrument.change) if instrument.change else 0.0
        ...
        logging.error(f"❌ Realtime price error for {symbol}: {str(e)}")
        return jsonify({'error': str(e)}), 500

# 📊 TRADINGVIEW DATAFEED API - Serves platform data to TradingView charts
@webtrader.route("/datafeed/config")
@login_required
def tradingview_config():
    ...
    return jsonify({
        ...
        'supports_time': True,
        'exchanges': [
            {'value': 'PLATFORM', 'name': 'Platform', 'desc': 'Platform Data'}
        ],
        'symbols_types': [
...
@webtrader.route("/datafeed/symbols")
@login_required
def tradingview_search_symbols():
    """Search symbols for TradingView"""
    query = request.args.get('query', '').upper()
    instruments = TradingInstrument.query.filter(
        TradingInstrument.symbol.like(f'%{query}%')
    ).limit(10).all()
    ...
    for instrument in instruments:
        results.append({
            'symbol': instrument.symbol,
            'full_name': f"PLATFORM:{instrument.symbol}",
...
@webtrader.route("/datafeed/symbol_info")
@login_required
def tradingview_symbol_info():
    """Get symbol info for TradingView"""
    symbol = request.args.get('symbol', '').replace('PLATFORM:', '')
    instrument = TradingInstrument.query.filter_by(symbol=symbol).first()
...
@webtrader.route("/datafeed/history")
@login_required
def tradingview_history():
    ...
    current_price = float(instrument.current_price) if instrument.current_price else 100.0
    
    # Generate some historical data based on current price
    # In production, you'd query actual historical data from your database
    import time
    current_time = int(time.time())
    
    # Generate 100 bars of fake historical data for demo
    bars = []
    base_time = current_time - (100 * 86400)  # 100 days ago
    
    for i in range(100):
        timestamp = base_time + (i * 86400)  # Daily bars
        if timestamp < from_timestamp:
            continue
        if timestamp > to_timestamp:
            break
            
        # Generate OHLC data with slight variations around current price
        variation = (i % 10 - 5) * 0.01  # ±5% variation
        open_price = current_price * (1 + variation)
        high_price = open_price * (1 + abs(variation) * 0.5)
        low_price = open_price * (1 - abs(variation) * 0.5)
        close_price = current_price * (1 + variation * 0.8)
